...
- 决策：
  - 单表映射 + 类型字段；**不**拆国家/洲物理表。类型初始预置 `country`/`continent`；类型编辑见补丁与 `add-customs-dict-types`。
  ...
  - 缺失由第三方写入 ZSET；本系统处理：正式同步**成功后**再 `ZREM`；可导出；标准字典导入见 `add-customs-dict-import`（与缺失导出同表头 upsert，导入不自动 ZREM）。
  - **无物理删除**；停用代替删除；**原始值创建后不可改**。
  - 搁置：处理历史、操作日志、整表覆盖全量、标准值强制列表。
- 后果：以 `add-customs-dict-mgmt` 交付；若日后确认仅本系统写正式 Hash，可再增加整表覆盖全量而不必改表结构。
- 补丁（2026-08-03）：解冻类型管理——表 `customs_dict_type`；可新建/改名/启停；编码不可改；有映射行不可停用；写路径校验启用类型（`add-customs-dict-types`）。
